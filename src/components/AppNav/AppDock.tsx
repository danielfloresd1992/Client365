...
import { visibleNavSections } from './navSections';
import { resolveAppDockTheme, type AppDockTheme } from './appDock.theme';
 ...
 * se resuelven por su clave con el registro (navigationIconRegistry).
 *
 *
 * ESTE ARCHIVO COMPONE, NO DECIDE
 *
 * Lo que antes vivía acá adentro y obligaba a editarlo por cualquier cambio se
 * repartió en tres piezas que se reemplazan sin tocarlo:
 *
 *   appDock.theme.ts          el aspecto — clases, tamaños, tintes
 *   navigationIconRegistry    qué icono es cada clave y cómo quiere tratarse
 *   navSections.ts            qué ve este usuario
 *   NavRow.tsx                una fila del menú
 *
 * Un icono nuevo, un color distinto o una regla de permisos más se agregan en
 * su archivo. AppDock no se entera, y ése es el punto.
 */

export default function AppDock({ theme: temaParcial }: { theme?: Partial<AppDockTheme> } = {}) {

    // Un tema parcial pisa solo lo que traiga; el resto queda como siempre.
    const theme = resolveAppDockTheme(temaParcial);
    ...
    // Qué ve este usuario. La regla vive en navSections.ts: es de permisos, se
    // prueba sin montar nada, y el día que aparezca otro criterio se agrega allá.
    const visibleSections = useMemo(
        () => visibleNavSections(NAV_SECTIONS, { isAdmin }),
        [isAdmin],
    );
        ...
        <div className='relative w-[52px] h-full shrink-0'>
            ...
            >
                ...
                <div className='flex-1 min-h-0 w-full flex flex-col overflow-y-auto overflow-x-hidden'>
                    {visibleSections.map(section => (
                        <div key={section.label} className='w-full pt-1'>
                            <p className={theme.sectionLabel}>{section.label}</p>
                            {section.items.map(item => (
                                <NavRow key={item.path} item={item} active={pathname === item.path} theme={theme} />
                            ))}
                        </div>
                    ))}
                </div>
                ...
                <div className='shrink-0 w-full pt-1.5 mt-1 border-t border-gray-100'>
                    ...
                    <NotificationBell rowClass={theme.row} iconBoxClass={theme.iconBox} labelClass={theme.label} />

                    {/* Usuario (avatar + nombre/rol). Si el horario de hoy lo
                        designa encargado/auxiliar, la campanita lo resalta. */}
                    <div className={`${theme.row} h-auto py-1.5 pointer-events-none`}>
                        <span className={`${theme.iconBox} relative`}>
                    ...
                    <button
                        ...
                        title='Configuración'
                        className={`${theme.row} w-[calc(100%-14px)] ${theme.actionRow}`}
                    >
                        <span className={theme.iconBox}><SettingsIcon size={theme.iconSize} /></span>
                        <span className={theme.label}>Configuración</span>
                    </button>
                    ...
                    <button
                        ...
                        title='Cerrar sesión'
                        className={`${theme.row} w-[calc(100%-14px)] ${theme.dangerRow}`}
                    >
                        <span className={theme.iconBox}><LogoutIcon size={theme.iconSize} /></span>
